        ...
        if n==3
            ...
            for j=1:nc
                ...
                if strcmp(p1.info,p2.info)
                    ...
                    %try 
                    mean_acent(2,tline-tstart+1)=mean_acent(2,tline-tstart+1)+rel';
                    %catch
                    %    dt=tline-tstart+1
                    %    relp=rel'
                    %    s=size(mean_acent) 
                    %    error('Index exceeds mat dim ?')
                    %end
                    sq_acent(2,tline-tstart+1)=sq_acent(2,tline-tstart+1)+rel'.^2;
                    ...
                    if na>1       
                        ...
                        [msd,merr,times,counts]=msd_untimed(ovangs);
                        ...
                        [msd,merr,times,counts]=msd_untimed(ovlens);
...
if ~isempty(angles)
    ...
    sq_ad(2,:)=sqrt(sqrt(sq_ad(2,:)./max(1,count_ad)-ang_diff(2,:).^2));
    mean_acent(2,:)=mean_acent(2,:)./max(1,count_acent);
    sq_acent(2,:)=sqrt(sq_acent(2,:)./max(1,count_acent)-mean_acent(2,:).^2);
    ...
    analysis.bipo_MSDang.name='MSD of bipolar spindle angle';
...
function [t0,te,m,n]=track_info(tp)
m=1;
n=0;
t0=666666666;
te=0;
